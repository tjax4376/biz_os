...
pub fn detect(snapshot: &Snapshot) -> Vec<Issue> {
    ...
    if snapshot.system.memory_total_kb > 0 {
        ...
        if used_pct >= 95 {
            issues.push(Issue {
                ...
                evidence: vec![format!("memory_used_percent={}%, used_kb={}, total_kb={}", used_pct, snapshot.system.memory_used_kb, snapshot.system.memory_total_kb)],
                remediation_suggestions: vec![
                    "Identify top memory consumers (ps/top) and stop non-essential workloads".to_string(),
    ...
    if total > 0 {
        ...
        if failed_pct >= 10 {
            issues.push(Issue {
                ...
                severity: if failed_pct >= 25 { Severity::Critical } else { Severity::Error },
                description: "A significant portion of AI requests are failing.".to_string(),
                evidence: vec![format!("failed_requests={}, total_requests={}, failed_percent={}%, gpu_errors={}", snapshot.ai_perf.failed_requests, total, failed_pct, snapshot.ai_perf.gpu_errors)],
                remediation_suggestions: vec![
                    "Inspect aiservd logs for model load/inference errors".to_string(),
                    "If GPU errors are increasing, consider forcing CPU inference temporarily".to_string(),
                ],
                first_seen_epoch_s: now,
                last_seen_epoch_s: now,
            });
    ...
    if snapshot.ai_perf.queue_depth_max >= 1000 {
        issues.push(Issue {
            ...
            severity: Severity::Warning,
            description: "AI request queue depth is high, suggesting saturation or downstream slowdowns.".to_string(),
            evidence: vec![format!("queue_depth_max={}, queue_depth_avg={}", snapshot.ai_perf.queue_depth_max, snapshot.ai_perf.queue_depth_avg)],
            remediation_suggestions: vec![
    ...
    if snapshot.ai_perf.gpu_errors > 0 {
        issues.push(Issue {
            ...
            evidence: vec![format!("gpu_errors={}, gpu_utilization_percent={}%, gpu_memory_used={}", snapshot.ai_perf.gpu_errors, snapshot.ai_perf.gpu_utilization_percent, snapshot.ai_perf.gpu_memory_used)],
            remediation_suggestions: vec![
